...
const Navbar = () => {
  ...
  return (
    <div
      className="navbar flex items-center justify-between bg-gradient-to-r from-[#caefd7] via-[#f5bfd7] to-[#abc9e9] fixed top-0 left-0 right-0 z-50 w-full h-16"
    ...
    >
      ...
      {!isScreenSize && (
        <div>
          {!(userName || userDetails?.admin_id) && (
            <ul className="navbar-menu flex gap-8">
              {["Home", "About-Us", "Book-A-Car", "Ride", "Reviews"].map(
                (item) => (
                  <li
                    key={item}
      ...
      <div className="navbar-right flex items-center">
        {userName ? (
           ...
           {isOpen && (
             <div className="absolute right-0 mt-2 w-36 bg-white shadow-lg rounded-lg p-4 border border-gray-200" style={{padding:"10px"}}>
               <span className="block text-gray-800 text-[17px] font-medium mb-2" style={{padding:"7px 0px"}}>
                 {userName}
               </span>
               <button
                 onClick={()=>setShowModal(true)}
                 className="w-full text-white p-2 text-[15px] bg-[#6f82c6] font-medium border-[#6f82c6] rounded-full hover:bg-gray-100 hover:text-black hover:border-[#6f82c6] transition-colors duration-300 ease-in-out transform hover:scale-105 shadow-md hover:shadow-lg"
                 style={{padding:"7px"}}
               >
                 Logout
               </button>
          ...
          <div className={`${isScreenSize ? "text-[13px]" : "text-[15px]"}`}>
            <button
              ...
              } border-[3px] bg-[#6f82c6] font-medium border-[#6f82c6] rounded-full hover:bg-gray-100 hover:text-black hover:border-[#6f82c6] transition-colors duration-300 ease-in-out transform hover:scale-105 shadow-md hover:shadow-lg`}
